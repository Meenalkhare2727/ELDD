...
MODULE_PARM_DESC(devcnt, "No. of pchar devices");
...
static __init int pchar_init(void) {
    ...
    devices = kmalloc(devcnt * sizeof(struct pchar_device), GFP_KERNEL);
    if (devices == NULL) {
        ret = -ENOMEM;
...
MODULE_DESCRIPTION("Simple pchar driver with kfifo as device and multiple instances support.");
